...
class TestDataManagerConfigClientRollback:
    ...
    @pytest.mark.asyncio
    async def test_rollback_config_success(self, mock_data_manager_client, config_manager):
    ...
    @pytest.mark.asyncio
    async def test_rollback_config_failure(self, mock_data_manager_client, config_manager):
    ...
    async def test_get_config_history(self):
        ...
        with patch('tradeengine.db.mongodb_client.DataManagerClient') as mock_client_class:
            mock_client_instance = AsyncMock()
            mock_client_instance.get_audit_trail = AsyncMock(return_value=mock_audit_records)
            ...
            history = await client.get_config_history(symbol="BTCUSDT", side="LONG", limit=10)
...
class TestTradingConfigManagerRollback:
    ...
    @pytest.mark.asyncio
    async def test_rollback_config_with_data_manager_support(self, config_manager, mock_mongodb_client):
    ...
    @pytest.mark.asyncio
    async def test_rollback_config_without_data_manager_support(self, config_manager, mock_mongodb_client):
