...
class TradingConfigManager:
    ...
    async def rollback_config(
        ...
        If both target_version and rollback_id are provided, rollback_id takes precedence.
            ...
            if rollback_id:
                ...
                if not params_to_restore:
                    return (
                        ...
                        None,
                        [
                            f"Audit record {rollback_id} either does not exist or does not match the requested scope {symbol or 'global'}{'-' + side if side else ''}"
                        ],
                    )
    ...
    async def get_config_by_id(
        ...
        if record:
            # Security validation: ensure audit record matches requested scope exactly
            record_symbol = record.get("symbol")
            record_side = record.get("side")

            if symbol != record_symbol:
                logger.warning(
                    f"Security: Audit ID {audit_id} symbol mismatch. "
                    f"Found: {record_symbol}, Requested: {symbol}"
                )
                return None

            if side != record_side:
                logger.warning(
                    f"Security: Audit ID {audit_id} side mismatch. "
                    f"Found: {record_side}, Requested: {side}"
                )
                return None
