...
from tradeengine.services.heartbeat_monitor import HeartbeatMonitor, HeartbeatMessage
...
async def test_heartbeat_monitor_lifecycle():
    ...
    with patch("nats.connect", new_callable=AsyncMock) as mock_connect:
        ...
        monitor = HeartbeatMonitor(nats_url="nats://localhost:4222", subject="test.heartbeat")
        
        await monitor.start()
        assert monitor.is_running is True
        mock_connect.assert_called_once()
        mock_nc.subscribe.assert_called_once_with("test.heartbeat", cb=monitor._message_handler)
