...
class PositionManager:
    ...
    def __init__(self):
        ...
        self.total_portfolio_value: float = 10000.0  # Placeholder, would integrate with account
        ...
        try:
            ...
            if order.side == "buy":
                ...
                if position["quantity"] > 0:
                    ...
                    if position["quantity"] <= 0:
                        # Position closed
                        logger.info(f"Position closed for {symbol}, total realized P&L: {position['realized_pnl']:.2f}")
            ...
            logger.info(f"Updated position for {symbol}: quantity={position['quantity']:.6f}, avg_price={position['avg_price']:.2f}")
            await audit_logger.log_position(position, status="updated")
        except Exception as e:
            logger.error(f"Error updating position for {symbol}: {e}")
            await audit_logger.log_error(str(e), context={"order": order.model_dump(), "result": result})
    ...
    async def check_position_limits(self, order: TradeOrder) -> bool:
        ...
        if order.position_size_pct and order.position_size_pct > self.max_position_size_pct:
            logger.warning(f"Position size {order.position_size_pct} exceeds limit {self.max_position_size_pct}")
            return False
        
        # Check portfolio exposure limit
        current_exposure = self._calculate_portfolio_exposure()
        if current_exposure > self.max_portfolio_exposure_pct:
            logger.warning(f"Portfolio exposure {current_exposure:.2%} exceeds limit {self.max_portfolio_exposure_pct:.2%}")
            return False
    ...
    async def check_daily_loss_limits(self) -> bool:
        ...
        if self.daily_pnl < -max_daily_loss:
            logger.warning(f"Daily loss {self.daily_pnl:.2f} exceeds limit {-max_daily_loss:.2f}")
            return False
    ...
    def set_risk_limits(self, max_position_size_pct: float, max_daily_loss_pct: float, max_portfolio_exposure_pct: float) -> None:
        """Set risk management limits"""
        self.max_position_size_pct = max_position_size_pct
        self.max_daily_loss_pct = max_daily_loss_pct
        self.max_portfolio_exposure_pct = max_portfolio_exposure_pct
        logger.info(f"Risk limits updated: position={max_position_size_pct:.1%}, daily_loss={max_daily_loss_pct:.1%}, exposure={max_portfolio_exposure_pct:.1%}")
