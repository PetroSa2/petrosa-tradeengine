#!/usr/bin/env python3
import os
import time

from binance import Client

# Initialize client
client = Client(
    os.getenv("BINANCE_API_KEY"), os.getenv("BINANCE_API_SECRET"), testnet=True
)
symbol = "BTCUSDT"
quantity = 0.001

print("=== GETTING CURRENT PRICE ===")
ticker = client.futures_symbol_ticker(symbol=symbol)
current_price = float(ticker["price"])
print(f"Current Price: ${current_price:,.2f}")

print("\n=== OPENING LONG POSITION ===")
try:
    position_result = client.futures_create_order(
        symbol=symbol, side="BUY", type="MARKET", quantity=quantity, positionSide="LONG"
    )
    print(f"Position opened: {position_result}")
except Exception as e:
    print(f"Position error: {e}")
    exit()

time.sleep(2)

print("\n=== PLACING TIGHT SL/TP ORDERS ===")
# Very tight orders - 0.1% and 0.2% from current price
sl_price = current_price * 0.999  # 0.1% below
tp_price = current_price * 1.002  # 0.2% above

print(f"SL Price: ${sl_price:,.2f} (0.1% below current)")
print(f"TP Price: ${tp_price:,.2f} (0.2% above current)")

# Place Stop Loss
try:
    sl_order = client.futures_create_order(
        symbol=symbol,
        side="SELL",
        type="STOP_MARKET",
        quantity=quantity,
        stopPrice=round(sl_price, 1),
        positionSide="LONG",
    )
    print(f"SL Order placed: {sl_order['orderId']}")
except Exception as e:
    print(f"SL error: {e}")

# Place Take Profit
try:
    tp_order = client.futures_create_order(
        symbol=symbol,
        side="SELL",
        type="TAKE_PROFIT_MARKET",
        quantity=quantity,
        stopPrice=round(tp_price, 1),
        positionSide="LONG",
    )
    print(f"TP Order placed: {tp_order['orderId']}")
except Exception as e:
    print(f"TP error: {e}")

print("\n=== FINAL STATUS ===")
# Show position
positions = client.futures_position_information(symbol=symbol)
for pos in positions:
    position_amt = float(pos["positionAmt"])
    if position_amt != 0:
        print(f"Position: {position_amt} BTC at ${pos['entryPrice']}")

# Show orders
orders = client.futures_get_open_orders(symbol=symbol)
print(f"Open Orders: {len(orders)}")
for order in orders:
    stop_price = float(order["stopPrice"])
    distance = abs(current_price - stop_price)
    pct = (stop_price / current_price - 1) * 100
    print(
        f"  {order['type']}: ${stop_price:,.2f} ({pct:+.2f}%) - Distance: ${distance:.2f}"
    )

print("\n=== READY FOR OCO TEST ===")
print("These tight orders should trigger quickly when price moves!")
