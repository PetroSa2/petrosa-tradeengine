# Petrosa Trading Engine Documentation Index

**Last Updated**: February 23, 2026
**Service Status**: ✅ ACTIVE (Production)

---

## 🚀 Getting Started
- [**README.md**](../README.md) - Project overview, architecture, and quick start.
- [**TESTNET_QUICK_START.md**](TESTNET_QUICK_START.md) - Guide for safe testing on Binance Testnet.

## 🏗️ Architecture & Operations
- [**ARCHITECTURE.md**](ARCHITECTURE.md) - System-wide architectural diagram and description.
- [**DEPLOYMENT.md**](DEPLOYMENT.md) - Production deployment and horizontal scaling.
- [**MANUAL_DEPLOYMENT_GUIDE.md**](MANUAL_DEPLOYMENT_GUIDE.md) - Procedures for manual overrides.
- [**TRADING_ENGINE_DOCUMENTATION.md**](TRADING_ENGINE_DOCUMENTATION.md) - Deep dive into order execution logic.

## 🔧 CI/CD & Development
- [**CI_CD_PIPELINE.md**](archive/2024-2025/CI_CD_PIPELINE.md) - (Reference) GitHub Actions workflow details.
- [**TESTING.md**](archive/2024-2025/TESTING.md) - (Reference) Test suite structure.
- [**MAKEFILE.md**](archive/2024-2025/MAKEFILE.md) - (Reference) Command reference.

## 📊 Risk & Metrics
- [**BUSINESS_METRICS.md**](BUSINESS_METRICS.md) - Trading execution metrics and monitoring.
- [**ALERTING_SETUP.md**](ALERTING_SETUP.md) - Configuration for trading alerts.
- [**HEDGE_MODE_POSITION_TRACKING.md**](HEDGE_MODE_POSITION_TRACKING.md) - Managing positions in hedge mode.
- [**POSITION_STATE_TRUTH_DECISION.md**](POSITION_STATE_TRUTH_DECISION.md) - Mirror vs always-consult decision for position/order truth (#549; root cause of -4130/-4509).

## 📜 Historical
- [**Archive Folder**](archive/) - Outdated or issue-specific documentation.
