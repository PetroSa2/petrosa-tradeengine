...
class TestDispatchCompletionPaths:
    ...
    @pytest.mark.skip(reason="Complex async monitoring test - error handling tested indirectly")
    @pytest.mark.asyncio
    ...
    async def test_oco_placement_failure_logs_errors(self, dispatcher):
        """Test that failed OCO placement logs errors (line 256-259)"""
        ...
        # Should return failed status and log errors
        assert result.get("status") == "failed"

    @pytest.mark.asyncio
    async def test_oco_placement_exception_logs_errors(self, dispatcher):
        """Test that OCO placement exception logs errors (line 261-263)"""
    ...
    @pytest.mark.asyncio
    async def test_oco_metrics_increment_on_success(self, dispatcher):
        """Test that OCO metrics are incremented on successful placement (line 237-240)"""
        ...
        with patch('tradeengine.dispatcher.strategy_position_manager') as mock_spm:
            mock_spm.get_strategy_position = Mock(return_value={
                "strategy_id": "test-strategy"
            })
